...
namespace UnrealSharpWeaver.MetaData
{
    ...
    {
        public FunctionMetaData(SyntaxNode member, EFunctionFlags functionFlags = EFunctionFlags.None) : base(member, "UFunctionAttribute")
        {
        ...
        public void RewriteMethodAsAsyncUFunctionImplementation(MethodDeclarationSyntax method)
        {
            // TODO: Implement the logic to rewrite the method as an async UFunction implementation.
            // No Async code in proof of concept yet. So this function will never be called.
        }

        public bool IsAsyncUFunction(MethodDeclarationSyntax method)
        {
            EFunctionFlags flags = GetFunctionFlags(method);

            return flags.HasFlag(EFunctionFlags.BlueprintCallable)  && method.ReturnType.ToString().StartsWith("System.Threading.Tasks.Task");
        }

        public EFunctionFlags GetFunctionFlags(MethodDeclarationSyntax method)
        {
            SemanticModel semanticModel = WeaveFile.semanticModel;
            var flags = EFunctionFlags.None;
            ...
            foreach (var attributeList in method.AttributeLists)
            {
                foreach (var attribute in attributeList.Attributes)
                {
                    ...
                    if (attrSymbol?.ContainingType.Name != "UFunction")
                        continue;

                    // ✅ Handle positional arguments like: [UFunction(FunctionFlags.BlueprintCallable)]
                    if (attribute.ArgumentList != null)
                    {
                        foreach (var arg in attribute.ArgumentList.Arguments)
                        {
                            ...
                            if (constant.HasValue && constant.Value is int intVal)
                            {
                                flags |= (EFunctionFlags)intVal;
                            }
                            else if (expr is MemberAccessExpressionSyntax memberAccess)
                            {
                                var symbol = semanticModel.GetSymbolInfo(memberAccess).Symbol;
                                if (symbol is IFieldSymbol field && field.ContainingType.Name == "FunctionFlags")
                                {
                                    flags |= (EFunctionFlags)field.ConstantValue!;
                                }
                            }
                        }
                    }

                    // Handle named arguments like: [UFunction(BlueprintCallable, Server)]
                    var attributeData = semanticModel.GetDeclaredSymbol(method)?
                        .GetAttributes()
                        .FirstOrDefault(a => a.AttributeClass?.Name == "UFunction");
                    ...
                    if (attributeData != null)
                    {
                        foreach (var namedArg in attributeData.NamedArguments)
                        {
                            ...
                            {
                                flags |= namedArg.Key switch
                                {
                                    "BlueprintCallable" => EFunctionFlags.BlueprintCallable,
                                    "Server" => EFunctionFlags.NetServer,
                                    "Client" => EFunctionFlags.NetClient,
                                    "NetMulticast" => EFunctionFlags.NetMulticast,
                                    "Exec" => EFunctionFlags.Exec,
                                    _ => EFunctionFlags.None
                                };
                            }
                        }
                    }
        ...
        public bool IsUFunction(MethodDeclarationSyntax methodDeclaration)
        {
            return methodDeclaration.AttributeLists
                .Select(al => al.Attributes)
                .Any(attr => attr.Any(attr2 => attr2.Name.ToString().Contains("UFunction")));
        }

        public bool IsInterfaceFunction(MethodDeclarationSyntax methodDeclaration)
        {
            return false; // TODO: Not implemented yet.
        }

        internal bool IsBlueprintEventOverride(MethodDeclarationSyntax methodDeclaration)
        {
            bool isOverride = methodDeclaration.Modifiers.Any(m => m.IsKind(Microsoft.CodeAnalysis.CSharp.SyntaxKind.OverrideKeyword));
            bool hasBlueprintEventAttr = methodDeclaration.AttributeLists
                .SelectMany(al => al.Attributes)
                .Any(attr => attr.Name.ToString().Contains("BlueprintEvent"));
            return isOverride && hasBlueprintEventAttr;
        }
        /*
